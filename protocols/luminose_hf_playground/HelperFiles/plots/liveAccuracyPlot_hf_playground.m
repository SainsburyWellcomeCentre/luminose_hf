function liveAccuracyPlot_hf_playground(ax, op, data)
    % liveAccuracyPlot_hf_playground - Moving average accuracy plot for playground task
    %
    % Syntax:
    %   liveAccuracyPlot_hf_playground(ax, 'init', [])     - Initialize the plot
    %   liveAccuracyPlot_hf_playground(ax, 'update', data) - Update with new trial data
    %
    % Inputs:
    ...
    switch op
        ...
        otherwise
            error('liveAccuracyPlot_hf_playground: invalid operation ''%s''. Use ''init'' or ''update''.', op);
    end
end
