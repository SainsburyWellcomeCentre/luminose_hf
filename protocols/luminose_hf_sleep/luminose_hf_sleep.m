function luminose_hf_sleep
    ...
    liveBarPlot_hf_sleep(BpodSystem.GUIHandles.AccuracyAxes, 'init', []);
    ...
    livePsychometricPlot_hf_sleep(BpodSystem.GUIHandles.PsychometricAxes, 'init', [], [1]);

    % Initialize Bpod notebook (for manual data annotation)                                                          
    BpodNotebook('init'); 

    % Initialize parameter GUI plugin
    LuminoseParameterGUI('init', S);
    ...
    if ~emulator
        ...
        for currentTrial = 1:S.GUI.maxTrials
            ...
            S = LuminoseParameterGUI('sync', S); % Sync parameters with LuminoseParameterGUI plugin
            ...
            if ~isempty(fieldnames(RawEvents))
                ...
                liveBarPlot_hf_sleep(BpodSystem.GUIHandles.AccuracyAxes, 'update', BpodSystem.Data);
                
                livePsychometricPlot_hf_sleep(BpodSystem.GUIHandles.PsychometricAxes, 'update', ...
                    BpodSystem.Data, [1]);
...
function [sma, S] = PrepareStateMachine(S, trialTypes, currentTrial, ITI, currentTrialEvents, emulator)
    ...
    switch response
        case 'Lick'
            responseDetect = {'Port1In', goAction, 'Tup', noGoAction};
            thirdState = 'DeliverStim';
        case 'Rotary Encoder'
    ...
    switch emulator
        case true
            ...
            sma = AddState(sma, 'Name', 'getResponse', ...
                ...
                'StateChangeConditions', {'Port1In', goAction, 'Tup', noGoAction},...
        ...
        case false
            ...
            sma = AddState(sma, 'Name', 'ShowCue', ... % Turn on LED of port1. Wait for InitDelay seconds. Ensure that wheel does not move.
                'Timer', 0,...
                'StateChangeConditions', {'Tup', thirdState},...
                'OutputActions', cueAction); % '*' = push new thresholds to rotary encoder 'Z' = zero position
            sma = AddState(sma, 'Name', 'InitRE', ...
                'Timer', 0.2,...
                'StateChangeConditions', {'RotaryEncoder1_3', 'DeliverStim', 'RotaryEncoder1_4', 'DeliverStim'},...
