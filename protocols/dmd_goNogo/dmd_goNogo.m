function dmd_goNogo
    ...
    clc;
    global BpodSystem S dmd
    beep('off'); % native matlab error sounds OFF
    BpodSystem.SoftCodeHandlerFunction = 'SoftCodeHandler_dmd_goNogo';
    ...
    BpodSystem.FlexIOConfig.threshold1(chanLick) = 2; % In range 0-5
...
function [sma, S] = PrepareStateMachine(S, trialTypes, currentTrial, ITI, currentTrialEvents)
    ...
    sma = AddState(sma, 'Name', 'TrialStart', ...
        ...
        'OutputActions', {'HiFi1', '*', 'PWM3', S.GUI.LEDIntensity, 'RotaryEncoder1', ['#' 0], 'AnalogThreshEnable', 1, 'Serial3', ['#' 1]}); % set sound, light on, set RE, enable flexI/O threshold, sync analog input module
    sma = AddState(sma, 'Name', 'deliverPattern', ... 
        'Timer', S.GUI.PatternTime,...
        'StateChangeConditions', {'Tup', 'getResponse'},...
        'OutputActions', {'PWM3', S.GUI.LEDIntensity, 'SoftCode', trialTypes(currentTrial), 'Serial3', ['=' 0 'High']}); % light on, trigger DMD
