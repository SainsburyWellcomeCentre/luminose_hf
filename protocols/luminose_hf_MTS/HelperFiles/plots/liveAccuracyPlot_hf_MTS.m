function liveAccuracyPlot_hf_MTS(ax, op, data)
    % liveAccuracyPlot_hf_MTS - Moving average accuracy plot for the MTS task
    %
    % Syntax:
    %   liveAccuracyPlot_hf_MTS(ax, 'init', [])     - Initialize the plot
    %   liveAccuracyPlot_hf_MTS(ax, 'update', data) - Update with new trial data
    %
    % Inputs:
    ...
    switch op
        case 'init'
            ...
            % Moving average lines (Match, Non-match, Overall)
            BpodSystem.GUIHandles.AccuracyPlot.LineLeft = plot(ax, NaN, NaN, ...
                ...
                'DisplayName', sprintf('Match — win=%d', WINDOW));
            BpodSystem.GUIHandles.AccuracyPlot.LineRight = plot(ax, NaN, NaN, ...
                ...
                'DisplayName', sprintf('Non-match — win=%d', WINDOW));
        ...
        case 'update'
            ...
            correctMatch    = NaN(1, nTrials);
            correctNonMatch = NaN(1, nTrials);
            ...
            for i = 1:nTrials
                ...
                switch trialType
                    case 1  % Match — correct = Reward
                        isCorrect = hasReward;
                        correctMatch(i) = double(isCorrect);
                    case 2  % Non-match — correct = reached ITI without punishment
                        isCorrect = hasITI && ~hasPunishment;
                        correctNonMatch(i) = double(isCorrect);
                end
            ...
            maMatch    = movingAccuracy(correctMatch,    WINDOW);
            maNonMatch = movingAccuracy(correctNonMatch, WINDOW);

            % Update lines
            set(BpodSystem.GUIHandles.AccuracyPlot.LineOverall, 'XData', trialIdx, 'YData', maOverall  * 100);
            set(BpodSystem.GUIHandles.AccuracyPlot.LineLeft,    'XData', trialIdx, 'YData', maMatch    * 100);
            set(BpodSystem.GUIHandles.AccuracyPlot.LineRight,   'XData', trialIdx, 'YData', maNonMatch * 100);
        ...
        otherwise
            error('liveAccuracyPlot_hf_MTS: invalid operation ''%s''. Use ''init'' or ''update''.', op);
    end
end
